...
// to the player. Used in the heuristic.
...
int minimax(const int depth, const int color, const Board board) 
{
  ...
  for (int i = 0; i < num_moves; i++) {
    ...
    int result = -minimax(depth - 1, color, newBoard);
  ...
  return (depth == DEPTH) ? move_num : evaluation;
}

// Make the move on the board given the move number.
// return 1 if a move was made, or 0 if no move was available.
