...
/**
 * A 1-d Row Block solution.
 */
void * workerThread(void *lp)
{
    int task_id = *((int *) lp);
    int begin = (nsize * task_id) / task_num;
    int end = (nsize * (task_id + 1)) / task_num; 
    ...
    while (currow < nsize) {
        ...
        // perform the reduction. Each thread has a pre-defined begin and end.
        // (1-d row blocked)
        for (j = begin; j < end; j++) {
            pivotval = matrix[j][currow - 1];
            matrix[j][currow - 1] = 0.0;
            for (k = currow; k < nsize; k++) {
