...
/**
 * A 1-d Row Block Cyclic solution.
 */
void * workerThread(void *lp)
{
    int task_id = *((int *) lp);
    int begin = block_size * task_id;
    ...
    while (currow < nsize) {
        ...
        // Perform the reduction. 1D Row Block Cyclic implementation.
        for (j = begin; j < nsize; j += block_size * task_num) {                // for each block
            for (row = 0; row < block_size && (row + j) < nsize; row++) {       // for each row in the block
                pivotval = matrix[j + row][currow - 1];
                matrix[j + row][currow - 1] = 0.0;
                for (k = currow; k < nsize; k++) {                              // for each column in the row
                    matrix[j + row][k] -= pivotval * matrix[currow - 1][k];
                }
                R[j + row] -= pivotval * R[currow - 1];
            }
        }
    }
    return NULL;
